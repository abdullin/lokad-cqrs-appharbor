...
namespace SaaS.Envelope
{
    public abstract class AbstractDataSerializer : IDataSerializer
    {
        ...
        protected sealed class Formatter
        {
            ...
            public Formatter(string contractName, Func<Stream, object> deserializerDelegate,
                Action<object, Stream> serializeDelegate)
            {
        ...
        void Build()
        {
            foreach (var type in KnownTypes)
            {
                ...
                {
                    var msg = string.Format("Duplicate contract '{0}' being added to {1}", formatter.ContractName,
                        GetType().Name);
